...
export default function PatientSelector({
  ...
  const checkExistingPatient = async () => {
    try {
      ...
      const response = await fetch(
        APPOINTMENTS_ENDPOINTS.checkPatient(user.email),
        {
          headers: {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to check patient status",
      );
      setDebugInfo({
        stage: "api-call",
  ...
  const handleCreatePatient = async (patientData: any) => {
    try {
      ...
      const response = await fetch(
        APPOINTMENTS_ENDPOINTS.createPatient(providerid),
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
