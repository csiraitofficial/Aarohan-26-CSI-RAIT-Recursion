...
import { MEDICINE_ENDPOINTS } from "@/lib/config";
...
export default function MedicineReminderPage() {
  ...
  const fetchReminders = async () => {
    ...
    try {
      const res = await fetch(MEDICINE_ENDPOINTS.reminders, {
        method: "GET",
        headers: {
    ...
    try {
      ...
      if (permission === "granted") {
        ...
        const currentToken = await getToken(messaging, {
          vapidKey:
            "BHpwiaGustDMjbQDHATxieQc65Fp34jhEvyCCExYNHl-QxxzeAXa6DkTPR3r3W6unmcCaYRaRGUNl_864uY0TYY", // from Firebase Console
        });
        if (currentToken) {
          console.log("FCM Token:", currentToken);
          // Send this token to your backend to store in Supabase
          await fetch(MEDICINE_ENDPOINTS.updateFcmToken, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authToken}`,
  ...
  const handleEditMedicine = async (
    ...
    try {
      ...
      const res = await fetch(MEDICINE_ENDPOINTS.reminder(id.toString()), {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
