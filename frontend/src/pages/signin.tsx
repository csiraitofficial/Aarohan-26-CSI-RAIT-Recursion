...
export default function AuthPage() {
    ...
    try {
      if (isLogin) {
        ...
        if (!registerResponse.ok) {
          ...
          const errorMsg = errorData?.error || registerResponse.statusText;
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Send user data to your backend
      const googleResponse = await fetch(AUTH_ENDPOINTS.google, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!googleResponse.ok) {
        ...
        const errorMsg = errorData?.error || googleResponse.statusText;
        throw new Error(`Google auth backend failed: ${errorMsg}`);
      }
