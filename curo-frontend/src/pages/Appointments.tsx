...
import { APPOINTMENTS_ENDPOINTS } from "@/lib/config";
...
export default function Appointments() {
  ...
  const [selectedAppointment, setSelectedAppointment] =
    useState<LocalAppointment | null>(null);
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const response = await fetch(APPOINTMENTS_ENDPOINTS.providers, {
        headers: {
      ...
      if (providerList.length === 0) {
        setDoctorError(
          data?.error || "No providers available right now. Please try again.",
        );
      }
    } catch (error) {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const token = await user.getIdToken();
      const response = await fetch(APPOINTMENTS_ENDPOINTS.get(user.email), {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleBookAppointment = async (appointmentData: any) => {
    try {
      ...
      const token = await user.getIdToken();
      const response = await fetch(APPOINTMENTS_ENDPOINTS.book(user.email), {
        method: "POST",
        headers: {
  ...
  const handleEditAppointment = async (appointmentData: any) => {
    try {
      ...
      const response = await fetch(
        APPOINTMENTS_ENDPOINTS.update(appointmentData.id),
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="max-w-7xl mx-auto p-6">
          {view === "list" ? (
            ...
            <AppointmentForm
              selectedDoctor={
                selectedAppointment ? selectedAppointment.provider : null
              }
