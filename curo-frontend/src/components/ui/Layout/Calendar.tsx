...
export default function CustomCalendar() {
  ...
  const fetchAppointments = async () => {
    ...
    const url = `${APPOINTMENTS_ENDPOINTS.get(uid || "")}`;
  ...
  const handleSaveAppointment = async (title: string) => {
    ...
    if ((!day_of_week && day_of_week !== 0) || !time || !user_id) {
      console.error("Missing required fields in currentSlot:", {
        day_of_week,
        time,
        user_id,
      });
    ...
    try {
      const response = await fetch(
        `${APPOINTMENTS_ENDPOINTS.book(uid || "")}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      console.log(
        "Appointments state updated with the new appointment:",
        savedAppointment,
      );
    } catch (error) {
                ...
                {Array.from({ length: 4 }, (_, quarter) => (
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      const nearestMinutes =
                        Math.round(offsetY / (cellHeight / 4)) * 15;
                  ...
                  >
                    {appointments
                      .filter(
                        (appt) =>
                          appt.day_of_week === dayIndex &&
                          appt.time === `${hour}:${quarter * 15}`,
                      )
                      .map((appt, i) => (
