...
const FacilityButton = ({
  ...
  >
    <div className="flex flex-col items-center space-y-3">
      <facility.icon
        className={`w-8 h-8 ${facility.color} transition-transform group-hover:scale-110 duration-300`}
      />
...
const FindServices = () => {
  ...
  const fetchPlaces = async (type: string, lat: number, lng: number) => {
    ...
    try {
      ...
      const response = await fetch(
        MAPS_ENDPOINTS.nearby(type, lat, lng, radius),
        {
          headers: {
  ...
  const fetchDoctors = async (keyword: string, lat: number, lng: number) => {
    ...
    try {
      ...
      const token = await user.getIdToken();
      const response = await fetch(
        MAPS_ENDPOINTS.doctorType(lat, lng, radius, keyword),
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (!response.ok) {
        const errBody = await response.json().catch(() => ({}));
        throw new Error(
          errBody?.details || errBody?.error || "Failed to fetch doctors",
        );
      }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600">
            Connect with healthcare professionals and medical facilities in your
            area
          </p>
        </div>
        ...
        <div
          ref={mapRef}
          className="w-full h-[400px] rounded-xl shadow-lg mb-8 scroll-mt-24"
        >
          {!mapsReady && (
            <div className="h-full w-full bg-gray-50 rounded-xl flex items-center justify-center text-gray-600">
              Loading map...
            </div>
          )}
        </div>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="text-sm text-gray-600">
                Showing {startIndex + 1}-{Math.min(endIndex, places.length)} of{" "}
                {places.length}
              </div>
            </div>
        ...
        {!loading && !error && userLocation && places.length === 0 && (
          <div className="bg-blue-50 border border-blue-200 text-blue-700 rounded-lg p-4">
            No medical services found for this selection. Try a larger radius or another category.
          </div>
        )}
